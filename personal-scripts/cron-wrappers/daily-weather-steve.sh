...
if [ -n "$TEXT_OUTPUT" ]; then
    if [ -n "$MEDIA_PATH" ] && [ -f "$MEDIA_PATH" ]; then
        "$OPENCLAW" message send --channel telegram --account steve --target 1191367022 --message "$TEXT_OUTPUT" --media "$MEDIA_PATH" 2>&1
    else
        "$OPENCLAW" message send --channel telegram --account steve --target 1191367022 --message "$TEXT_OUTPUT" 2>&1
    fi
fi
