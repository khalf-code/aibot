...
read_when:
  - Designing workspace memory (~/.zoidbergbot/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep ZoidbergBot integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.zoidbergbot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.zoidbergbot/workspace` as canonical human-readable memory.
...
Recommendation: **deep integration in ZoidbergBot**, but keep a separable core library.

### Why integrate into ZoidbergBot?

- ZoidbergBot already knows:
...
Pragmatic take for `~/.zoidbergbot/workspace`:
