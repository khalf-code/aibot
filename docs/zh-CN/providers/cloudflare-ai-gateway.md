...
Cloudflare AI Gateway 位于提供商 API 之前，让你可以添加分析、缓存和控制功能。对于 Anthropic，OpenClaw 通过你的 Gateway 端点使用 Anthropic Messages API。
...
如果你在 Cloudflare 中启用了 Gateway 认证，请添加 `cf-aig-authorization` 头（这是在提供商 API 密钥之外的额外认证）。
