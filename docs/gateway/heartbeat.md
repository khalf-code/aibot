...
- During heartbeat runs, ZoidbergBot treats `HEARTBEAT_OK` as an ack when it appears
...
If **all three** are false, ZoidbergBot skips the heartbeat run entirely (no model call).
