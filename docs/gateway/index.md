...
- Replaces the legacy `gateway` command. CLI entry point: `zoidbergbot gateway`.
...
- Config hot reload watches `~/.zoidbergbot/zoidbergbot.json` (or `ZOIDBERGBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__zoidbergbot__/canvas/` from `~/.zoidbergbot/workspace/canvas`. Disable with `canvasHost.enabled=false` or `ZOIDBERGBOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `ZOIDBERGBOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `ZOIDBERGBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.zoidbergbot.*` may still exist)
- Linux: `zoidbergbot-gateway-<profile>.service`
- Windows: `ZoidbergBot Gateway (<profile>)`
...
- `ZOIDBERGBOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.zoidbergbot/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `ZOIDBERGBOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `ZOIDBERGBOT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `ZOIDBERGBOT_CONFIG_PATH`
- unique `ZOIDBERGBOT_STATE_DIR`
...
- Use launchd to keep the service alive:
  - Program: path to `zoidbergbot`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `zoidbergbot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.zoidbergbot.*` is cleaned up).
  - `zoidbergbot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are ZoidbergBot profile services.
  ...
  - Cleanup: `zoidbergbot gateway uninstall` (current service) and `zoidbergbot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `zoidbergbot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- ZoidbergBot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.zoidbergbot.*` labels still unload cleanly).
- To stop it cleanly, use `zoidbergbot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `zoidbergbot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `zoidbergbot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

ZoidbergBot installs a **systemd user service** by default on Linux/WSL2. We
...
`zoidbergbot gateway install` writes the user unit. `zoidbergbot doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/zoidbergbot-gateway[-<profile>].service`:

```
[Unit]
Description=ZoidbergBot Gateway (profile: <profile>, v<version>)
...
Create `/etc/systemd/system/zoidbergbot-gateway[-<profile>].service` (copy the unit above,
...
- `zoidbergbot gateway health|status` — request health/status over the Gateway WS.
- `zoidbergbot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `zoidbergbot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `zoidbergbot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `zoidbergbot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `zoidbergbot gateway` and the legacy TCP control port.
