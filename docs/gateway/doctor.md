...
`zoidbergbot doctor` is the repair + migration tool for ZoidbergBot. It fixes stale
...
- Extra workspace dir detection (`~/zoidbergbot`).
...
you to run `zoidbergbot doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.zoidbergbot/zoidbergbot.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.zoidbergbot/sessions/` to `~/.zoidbergbot/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.zoidbergbot/agent/` to `~/.zoidbergbot/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.zoidbergbot/credentials/*.json` (except `oauth.json`)
  - to `~/.zoidbergbot/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `zoidbergbot doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.zoidbergbot` folders exist across
  home directories or when `ZOIDBERGBOT_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.zoidbergbot/zoidbergbot.json` is
...
offers to remove them and install the ZoidbergBot service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named ZoidbergBot gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `zoidbergbot doctor --generate-gateway-token` to force token
...
- `zoidbergbot doctor` prompts before rewriting supervisor config.
- `zoidbergbot doctor --yes` accepts the default repair prompts.
- `zoidbergbot doctor --repair` applies recommended fixes without prompts.
- `zoidbergbot doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `zoidbergbot gateway install --force`.
