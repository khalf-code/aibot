...
read_when:
  - "You installed ZoidbergBot but `zoidbergbot` is “command not found”"
...
ZoidbergBot’s runtime baseline is **Node 22+**.

If you can run `npm install -g zoidbergbot@latest` but later see `zoidbergbot: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
