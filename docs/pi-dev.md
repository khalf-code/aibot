...
This guide summarizes a sane workflow for working on the pi integration in ZoidbergBot.
...
State lives under the ZoidbergBot state directory. Default is `~/.zoidbergbot`. If `ZOIDBERGBOT_STATE_DIR` is set, use that directory instead.

To reset everything:

- `zoidbergbot.json` for config
