---
summary: "Use MiniMax M2.1 in ZoidbergBot"
read_when:
  - You want MiniMax models in ZoidbergBot
...
```bash
zoidbergbot plugins enable minimax-portal-auth  # skip if already loaded.
zoidbergbot gateway restart  # restart if gateway is already running
...
See [MiniMax OAuth plugin README](https://github.com/zoidbergbot/zoidbergbot/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `zoidbergbot.json`:
...
- Use `zoidbergbot models list` and `zoidbergbot models set minimax/MiniMax-M2.1` to switch.
...
- Running `zoidbergbot configure` and selecting **MiniMax M2.1**, or
