---
summary: "Use Synthetic's Anthropic-compatible API in ZoidbergBot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. ZoidbergBot registers it as the
...
Note: ZoidbergBot's Anthropic client appends `/v1` to the base URL, so use
