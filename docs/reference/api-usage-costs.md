...
ZoidbergBot features that can generate provider usage or paid API calls.
...
- `zoidbergbot status --usage` and `zoidbergbot channels list` show provider **usage windows**
...
ZoidbergBot can pick up credentials from:
...
`zoidbergbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
