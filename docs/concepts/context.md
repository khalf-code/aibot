...
read_when:
  - You want to understand what “context” means in ZoidbergBot
...
“Context” is **everything ZoidbergBot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (ZoidbergBot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How ZoidbergBot builds the system prompt

The system prompt is **ZoidbergBot-owned** and rebuilt each run. It includes:
...
By default, ZoidbergBot injects a fixed set of workspace files (if present):
