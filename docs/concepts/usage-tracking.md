...
- `/usage cost` in chats: local cost summary aggregated from ZoidbergBot session logs.
- CLI: `zoidbergbot status --usage` prints a full per-provider breakdown.
- CLI: `zoidbergbot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
