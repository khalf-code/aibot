...
You can also set `ZOIDBERGBOT_VERBOSE=1` instead of the flag.
...
### `docs.zoidbergbot.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.zoidbergbot.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.zoidbergbot.ai` to the allowlist, then retry.
...
- Check the allowlist: `zoidbergbot config get agents.defaults.models`
...
If you can, include the relevant log tail from `zoidbergbot logs --follow`.
