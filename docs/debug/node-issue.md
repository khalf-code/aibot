...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ZoidbergBot
...
Running ZoidbergBot via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `zoidbergbot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node zoidbergbot.mjs status` works on Node 25.
