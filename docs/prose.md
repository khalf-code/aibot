---
summary: "OpenProse: .prose workflows, slash commands, and state in ZoidbergBot"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In ZoidbergBot it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
Dev/local checkout: `zoidbergbot plugins install ./extensions/open-prose`

Related docs: [Plugins](/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses ZoidbergBot tools under the hood.
...
## ZoidbergBot runtime mapping

OpenProse programs map to ZoidbergBot primitives:

| OpenProse concept         | ZoidbergBot tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use ZoidbergBot tool allowlists and approval gates to control side effects.
