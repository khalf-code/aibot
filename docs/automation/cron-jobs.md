...
- Jobs persist under `~/.zoidbergbot/cron/` so restarts don’t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.zoidbergbot/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `zoidbergbot cron add/edit` or the cron
...
- Job store: `~/.zoidbergbot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.zoidbergbot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
- Check cron is enabled: `cron.enabled` and `ZOIDBERGBOT_SKIP_CRON`.
