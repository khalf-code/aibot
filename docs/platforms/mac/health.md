...
- You can still use the CLI flow in [Gateway health](/gateway/health) (`zoidbergbot status`, `zoidbergbot status --deep`, `openclaw health --json`) and tail `/tmp/zoidbergbot/zoidbergbot-*.log` for `web-heartbeat` / `web-reconnect`.
