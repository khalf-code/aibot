...
service via the external `zoidbergbot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`ZOIDBERGBOT_PROFILE`; legacy `com.zoidbergbot.*` is supported).
...
- Writes `~/.zoidbergbot/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.zoidbergbot/disable-launchagent`,
