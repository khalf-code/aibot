...
1.  **The `status` Command Anchor**: `src/cli/program/command-registry.ts` eagerly imports `statusCommand`.
    ...
    - Impact: This pulls in the entire channel plugin system and browser automation config for _every_ CLI command.
...
1.  **Hidden Eager Imports**: While the _registry_ was lazy (dynamic imports inside `register()` functions), the **registration modules themselves** (`src/cli/program/register.*.ts`) were still being imported sequentially by `registerProgramCommands`.
...
3.  **The Impact**: `src/cli/deps.ts` imports the entire channel delivery stack (`channels/web`, `discord/send`, `slack/send`, etc.). This meant that simply _building_ the commander program (to show help or version) triggered the loading of all channel libraries (Discord.js, Puppeteer config, etc.).
...
- **Current**: Scans _all_ files to find the absolute latest mtime.
