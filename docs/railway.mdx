...
Deploy ZoidbergBot on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/zoidbergbot` — Control UI
...
- `ZOIDBERGBOT_STATE_DIR=/data/.openclaw` (recommended)
- `ZOIDBERGBOT_WORKSPACE_DIR=/data/workspace` (recommended)
- `ZOIDBERGBOT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your ZoidbergBot state + workspace so you can migrate to another host without losing config or memory.
