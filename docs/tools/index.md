---
summary: "Agent tool surface for ZoidbergBot (browser, canvas, nodes, message, cron) replacing legacy `zoidbergbot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `zoidbergbot-*` skills
title: "Tools"
---

# Tools (ZoidbergBot)

ZoidbergBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `zoidbergbot-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `zoidbergbot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, ZoidbergBot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`zoidbergbot node run`).
...
- Requires a Brave API key (recommended: `zoidbergbot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated ZoidbergBot-managed browser.
...
- Quick smoke: `zoidbergbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `zoidbergbot gateway` restart in-place)
