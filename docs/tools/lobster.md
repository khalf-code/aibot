---
title: Lobster
summary: "Typed workflow runtime for ZoidbergBot with resumable approval gates."
...
Lobster is a workflow shell that lets ZoidbergBot run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: ZoidbergBot runs one Lobster tool call and gets a structured result.
...
ZoidbergBot launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In ZoidbergBot tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the ZoidbergBot Gateway (see the [Lobster repo](https://github.com/zoidbergbot/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), ZoidbergBot keeps core tools enabled. To restrict core
...
→ zoidbergbot calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ zoidbergbot calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls ZoidbergBot tools that do.
