...
struct IOSGatewayChatTransport: ZoidbergBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ZoidbergBotChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ZoidbergBotChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ZoidbergBotChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ZoidbergBotChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ZoidbergBotChatAttachmentPayload]) async throws -> ZoidbergBotChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ZoidbergBotChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(ZoidbergBotChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(ZoidbergBotGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ZoidbergBotGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ZoidbergBotChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ZoidbergBotAgentEventPayload.self)
                        {
