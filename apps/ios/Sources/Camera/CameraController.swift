...
actor CameraController {
    ...
    func snap(params: ZoidbergBotCameraSnapParams) async throws -> (
        format: String,
        base64: String,
        width: Int,
        height: Int)
    ...
    func clip(params: ZoidbergBotCameraClipParams) async throws -> (
        format: String,
        base64: String,
        durationMs: Int,
        hasAudio: Bool)
    {
        ...
        let movURL = FileManager().temporaryDirectory
            .appendingPathComponent("zoidbergbot-camera-\(UUID().uuidString).mov")
        let mp4URL = FileManager().temporaryDirectory
            .appendingPathComponent("zoidbergbot-camera-\(UUID().uuidString).mp4")
    ...
    private nonisolated static func pickCamera(
        facing: ZoidbergBotCameraFacing,
