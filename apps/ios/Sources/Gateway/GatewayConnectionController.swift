...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [ZoidbergBotCapability.canvas.rawValue, ZoidbergBotCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(ZoidbergBotCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(ZoidbergBotCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = ZoidbergBotLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(ZoidbergBotCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            ZoidbergBotSystemCommand.notify.rawValue,
            ZoidbergBotSystemCommand.which.rawValue,
            ZoidbergBotSystemCommand.run.rawValue,
            ZoidbergBotSystemCommand.execApprovalsGet.rawValue,
            ZoidbergBotSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(ZoidbergBotCapability.camera.rawValue) {
            commands.append(ZoidbergBotCameraCommand.list.rawValue)
            commands.append(ZoidbergBotCameraCommand.snap.rawValue)
            commands.append(ZoidbergBotCameraCommand.clip.rawValue)
        }
        if caps.contains(ZoidbergBotCapability.location.rawValue) {
            commands.append(ZoidbergBotLocationCommand.get.rawValue)
        }
