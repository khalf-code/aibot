...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = ZoidbergBotProtocol.AnyCodable(["triggers": ["zoidbergbot", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["zoidbergbot", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = ZoidbergBotProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
