...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ZoidbergBotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ZoidbergBotProtocol.AnyCodable] = [
            "presence": .init([ZoidbergBotProtocol.AnyCodable(entry)]),
        ]
        let payload = ZoidbergBotProtocol.AnyCodable(payloadMap)
