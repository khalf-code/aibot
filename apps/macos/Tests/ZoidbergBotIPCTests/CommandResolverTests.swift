...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersZoidbergBotBinary() async throws {
        ...
        let zoidbergbotPath = tmp.appendingPathComponent("node_modules/.bin/zoidbergbot")
        try self.makeExec(at: zoidbergbotPath)

        let cmd = CommandResolver.openclawCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([zoidbergbotPath.path, "gateway"]))
    }
    ...
    @Test func fallsBackToNodeAndScript() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/zoidbergbot.js")
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "zoidbergbot", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "zoidbergbot", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() async throws {
        ...
        defaults.set("zoidbergbot@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/zoidbergbot", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "zoidbergbot@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/zoidbergbot'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("zoidbergbot"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() async throws {
        ...
        #expect(cmd.first == zoidbergbotPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
