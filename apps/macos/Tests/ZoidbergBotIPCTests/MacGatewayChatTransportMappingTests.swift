...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: ZoidbergBotProtocol.AnyCodable(["ok": ZoidbergBotProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: ZoidbergBotProtocol.AnyCodable(["ok": ZoidbergBotProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = ZoidbergBotProtocol.AnyCodable([
            "runId": ZoidbergBotProtocol.AnyCodable("run-1"),
            "sessionKey": ZoidbergBotProtocol.AnyCodable("main"),
            "state": ZoidbergBotProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: ZoidbergBotProtocol.AnyCodable(["a": ZoidbergBotProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
