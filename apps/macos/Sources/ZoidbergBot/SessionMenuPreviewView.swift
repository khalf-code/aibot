...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.zoidbergbot", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ZoidbergBotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ZoidbergBotSessionPreviewEntry,
    ...
    private static func cache(payload: ZoidbergBotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ZoidbergBotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ZoidbergBotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ZoidbergBotKit.AnyCodable]) -> [ZoidbergBotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ZoidbergBotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ZoidbergBotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ZoidbergBotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ZoidbergBotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ZoidbergBotChatMessage) -> String? {
