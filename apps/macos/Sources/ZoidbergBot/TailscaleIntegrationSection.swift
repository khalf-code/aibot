...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.zoidbergbot/zoidbergbot.json. Prefer ZOIDBERGBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.zoidbergbot/zoidbergbot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.zoidbergbot/zoidbergbot.json. Restart the gateway to apply."
        }
