...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ZoidbergBotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.zoidbergbot", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ZoidbergBotKit.AnyCodable]()) {
                $0[$1.key] = ZoidbergBotKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ZoidbergBotProtocol.AnyCodable?) -> [String: ZoidbergBotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ZoidbergBotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ZoidbergBotKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ZoidbergBotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("zoidbergbot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("zoidbergbot.control.agent")
}
