...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.zoidbergbot", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.zoidbergbot", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [ZoidbergBotChatMessage] = messages.compactMap { item in
