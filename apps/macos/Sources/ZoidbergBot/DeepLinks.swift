...
private let deepLinkLogger = Logger(subsystem: "ai.zoidbergbot", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "ZoidbergBot is paused", message: "Unpause ZoidbergBot to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run ZoidbergBot agent?", message: body) else { return }
        }
