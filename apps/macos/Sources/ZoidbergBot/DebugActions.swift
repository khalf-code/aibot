...
enum DebugActions {
    private static let verboseDefaultsKey = "zoidbergbot.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "zoidbergbot.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ZoidbergBotPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "ZoidbergBot", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ZoidbergBotPaths.configURL
        guard
