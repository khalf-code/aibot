...
enum CommandResolver {
    private static let projectRootDefaultsKey = "zoidbergbot.gatewayProjectRootPath"
    private static let helperName = "zoidbergbot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let openclawEntry = root.appendingPathComponent("zoidbergbot.mjs").path
        if FileManager().isReadableFile(atPath: openclawEntry) { return openclawEntry }
        let binEntry = root.appendingPathComponent("bin/zoidbergbot.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/zoidbergbot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let zoidbergbotPaths = self.openclawManagedPaths(home: home)
        if !zoidbergbotPaths.isEmpty {
            extras.insert(contentsOf: zoidbergbotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + zoidbergbotPaths.count)
        var seen = Set<String>()
    ...
    private static func openclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".zoidbergbot"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func projectZoidbergBotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("zoidbergbot.mjs").path,
            root.appendingPathComponent("bin/zoidbergbot.js").path,
        ]
    ...
    static func hasAnyZoidbergBotInvoker(searchPaths: [String]? = nil) -> Bool {
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let zoidbergbotPath = self.projectZoidbergBotExecutable(projectRoot: root) {
                return [zoidbergbotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "zoidbergbot", subcommand] + extraArgs
            }
            if let zoidbergbotPath = self.openclawExecutable(searchPaths: searchPaths) {
                return [zoidbergbotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            zoidbergbot entrypoint missing (looked for dist/index.js or zoidbergbot.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real zoidbergbot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/zoidbergbot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v zoidbergbot >/dev/null 2>&1; then
          CLI="$(command -v zoidbergbot)"
          zoidbergbot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/zoidbergbot.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/zoidbergbot.mjs"
            node "$PRJ/zoidbergbot.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/zoidbergbot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/zoidbergbot.js"
            node "$PRJ/bin/zoidbergbot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent openclaw"
          pnpm --silent zoidbergbot \(quotedArgs);
        else
    ...
    {
        let root = configRoot ?? ZoidbergBotConfigFile.loadDict()
