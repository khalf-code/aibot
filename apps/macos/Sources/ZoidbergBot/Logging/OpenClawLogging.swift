...
enum ZoidbergBotLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = ZoidbergBotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ZoidbergBotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ZoidbergBotLogging.bootstrapIfNeeded()
        let label = ZoidbergBotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ZoidbergBotOSLogHandler: LogHandler {
...
struct ZoidbergBotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ZoidbergBotLogging.parseLabel(self.label)
