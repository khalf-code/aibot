...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ZOIDBERGBOT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/zoidbergbot")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("zoidbergbot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("zoidbergbot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/zoidbergbot/ (rolling or stdout), or nil if none exist.
