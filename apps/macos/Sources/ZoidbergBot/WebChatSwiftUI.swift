...
private let webChatSwiftLogger = Logger(subsystem: "ai.zoidbergbot", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ZoidbergBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ZoidbergBotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ZoidbergBotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ZoidbergBotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ZoidbergBotGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    ZoidbergBotChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    ZoidbergBotAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ZoidbergBotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ZoidbergBotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ZoidbergBotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ZoidbergBotChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ZoidbergBotChatView>) -> NSViewController
    {
