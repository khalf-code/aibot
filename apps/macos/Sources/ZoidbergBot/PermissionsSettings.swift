...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so ZoidbergBot can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = ZoidbergBotLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = ZoidbergBotLocationMode.off.rawValue
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Picker("", selection: self.$locationModeRaw) {
                Text("Off").tag(ZoidbergBotLocationMode.off.rawValue)
                Text("While Using").tag(ZoidbergBotLocationMode.whileUsing.rawValue)
                Text("Always").tag(ZoidbergBotLocationMode.always.rawValue)
            }
            .labelsHidden()
            .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = ZoidbergBotLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: ZoidbergBotLocationMode {
        ZoidbergBotLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: ZoidbergBotLocationMode) async -> Bool {
