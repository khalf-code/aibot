...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.zoidbergbot", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ZoidbergBotPeekabooBridgeServices?
    private static var openclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("ZoidbergBot", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = ZoidbergBotPeekabooBridgeServices()
...
@MainActor
private final class ZoidbergBotPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.zoidbergbot.peekaboo")
