...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.zoidbergbot", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["ZOIDBERGBOT_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = ZoidbergBotConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["ZOIDBERGBOT_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "zoidbergbot@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "zoidbergbot@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "zoidbergbot@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "zoidbergbot@\(target)"])
            }

        statusHandler("Installing zoidbergbot@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed zoidbergbot@\(target)")
        } else {
