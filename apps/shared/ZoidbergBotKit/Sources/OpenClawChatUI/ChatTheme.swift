...
enum ZoidbergBotChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("ZoidbergBotChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("ZoidbergBotChatTheme.onboardingAssistantBubble"),
...
enum ZoidbergBotPlatformImageFactory {
    static func image(_ image: ZoidbergBotPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
