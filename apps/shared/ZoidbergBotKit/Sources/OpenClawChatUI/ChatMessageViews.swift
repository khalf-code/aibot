...
struct ChatMessageBubble: View {
    let message: ZoidbergBotChatMessage
    let style: ZoidbergBotChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ZoidbergBotChatTheme.userText : ZoidbergBotChatTheme.assistantText
    ...
    private var inlineAttachments: [ZoidbergBotChatMessageContent] {
    ...
    private var toolCalls: [ZoidbergBotChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ZoidbergBotChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ZoidbergBotChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ZoidbergBotChatTheme.onboardingAssistantBubble
        }
        return ZoidbergBotChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ZoidbergBotChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ZoidbergBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ZoidbergBotChatTheme.userText : ZoidbergBotChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ZoidbergBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ZoidbergBotChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ZoidbergBotChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ZoidbergBotChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ZoidbergBotChatTheme.assistantText)
            }
        }
    }
}
