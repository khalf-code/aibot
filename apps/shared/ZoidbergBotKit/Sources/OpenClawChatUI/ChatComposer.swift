...
@MainActor
struct ZoidbergBotChatComposer: View {
    @Bindable var viewModel: ZoidbergBotChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ZoidbergBotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ZoidbergBotChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ZoidbergBotPendingAttachment.id)
                { (att: ZoidbergBotPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ZoidbergBotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(ZoidbergBotChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ZoidbergBotChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ZoidbergBotChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ZoidbergBotChatTheme.subtleCard)
        .clipShape(Capsule())
    }
