...
private let chatUILogger = Logger(subsystem: "ai.zoidbergbot", category: "ZoidbergBotChatUI")

@MainActor
@Observable
public final class ZoidbergBotChatViewModel {
    public private(set) var messages: [ZoidbergBotChatMessage] = []
    ...
    public var attachments: [ZoidbergBotPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ZoidbergBotChatPendingToolCall] = []
    public private(set) var sessions: [ZoidbergBotChatSessionEntry] = []
    private let transport: any ZoidbergBotChatTransport
    ...
    private var pendingToolCallsById: [String: ZoidbergBotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ZoidbergBotChatTransport) {
    ...
    public var sessionChoices: [ZoidbergBotChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [ZoidbergBotChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [ZoidbergBotChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ZoidbergBotPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ZoidbergBotChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ZoidbergBotChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [ZoidbergBotChatMessage]) -> [ZoidbergBotChatMessage] {
        var result: [ZoidbergBotChatMessage] = []
    ...
    private static func dedupeKey(for message: ZoidbergBotChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ZoidbergBotChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> ZoidbergBotChatAttachmentPayload in
            ZoidbergBotChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ZoidbergBotChatSessionEntry {
        ZoidbergBotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ZoidbergBotChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ZoidbergBotChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ZoidbergBotAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ZoidbergBotChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ZoidbergBotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
