...
import Foundation
import Testing
@testable import ZoidbergBotChatUI
...
private final class TestChatTransport: @unchecked Sendable, ZoidbergBotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ZoidbergBotChatHistoryPayload]
    private let sessionsResponses: [ZoidbergBotChatSessionsListResponse]

    private let stream: AsyncStream<ZoidbergBotChatTransportEvent>
    private let continuation: AsyncStream<ZoidbergBotChatTransportEvent>.Continuation

    init(
        historyResponses: [ZoidbergBotChatHistoryPayload],
        sessionsResponses: [ZoidbergBotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ZoidbergBotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ZoidbergBotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ZoidbergBotChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [ZoidbergBotChatAttachmentPayload]) async throws -> ZoidbergBotChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return ZoidbergBotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ZoidbergBotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ZoidbergBotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ZoidbergBotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ZoidbergBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ZoidbergBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ZoidbergBotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ZoidbergBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ZoidbergBotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ZoidbergBotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ZoidbergBotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
