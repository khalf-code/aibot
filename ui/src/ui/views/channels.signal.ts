...
export function renderSignalCard(params: {
  ...
  return html`
    <div class="card">
      ...
      <div class="card-sub">${t("signal.subtitle")}</div>
      ${accountCountLabel}
      ...
      <div class="status-list" style="margin-top: 16px;">
        ...
        <div>
          <span class="label">${t("signal.baseUrl")}</span>
        ...
        signal?.probe
          ? html`<div class="callout" style="margin-top: 12px;">
            ${t("channels.probeStatus", {
              status: signal.probe.ok ? t("common.valid") : t("common.invalid"),
              message: (signal.probe.status ?? "") + (signal.probe.error ?? ""),
            })}
          </div>`
          : nothing
      }
