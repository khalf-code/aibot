...
import type { ZoidbergBotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: ZoidbergBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("zoidbergbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("zoidbergbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("zoidbergbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("zoidbergbot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("zoidbergbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
