...
vi.mock("../infra/zoidbergbot-root.js", () => ({
  resolveZoidbergBotPackageRoot: vi.fn().mockResolvedValue("/tmp/zoidbergbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/zoidbergbot/pnpm-lock.yaml",
      markerPath: "/tmp/zoidbergbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("ZoidbergBot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("zoidbergbot status --all") ||
          l.includes("zoidbergbot --profile isolated status --all") ||
          l.includes("zoidbergbot status --all") ||
          l.includes("zoidbergbot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ZOIDBERGBOT_GATEWAY_TOKEN = "abcd1234";
    try {
