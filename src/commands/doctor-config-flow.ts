...
function stripUnknownConfigKeys(config: ZoidbergBotConfig): {
  config: ZoidbergBotConfig;
  removed: string[];
} {
  const parsed = ZoidbergBotSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: ZoidbergBotConfig) {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".zoidbergbot");
  const targetPath = path.join(targetDir, "zoidbergbot.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: ZoidbergBotConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("zoidbergbot doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("zoidbergbot doctor --fix")}" to apply these changes.`,
      );
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "zoidbergbot doctor --fix" to remove these keys.');
    }
  }
