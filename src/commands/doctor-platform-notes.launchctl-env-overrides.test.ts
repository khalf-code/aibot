...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ZOIDBERGBOT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ZOIDBERGBOT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ZOIDBERGBOT_GATEWAY_TOKEN");
    expect(message).not.toContain("ZOIDBERGBOT_GATEWAY_PASSWORD");
  });
