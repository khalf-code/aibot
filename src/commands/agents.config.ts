...
export function listAgentEntries(cfg: ZoidbergBotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ZoidbergBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: ZoidbergBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ZoidbergBotConfig): AgentSummary[] {
