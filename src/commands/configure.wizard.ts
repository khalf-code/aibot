    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from zoidbergbot.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "ZoidbergBot update wizard" : "ZoidbergBot configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: ZoidbergBotConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("zoidbergbot doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env.ZOIDBERGBOT_GATEWAY_TOKEN;
        const password =
          nextConfig.gateway?.auth?.password ?? process.env.ZOIDBERGBOT_GATEWAY_PASSWORD;
    ...
    const newPassword =
      nextConfig.gateway?.auth?.password ?? process.env.ZOIDBERGBOT_GATEWAY_PASSWORD;
    const oldPassword =
      baseConfig.gateway?.auth?.password ?? process.env.ZOIDBERGBOT_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.ZOIDBERGBOT_GATEWAY_TOKEN;
