...
function resolveMode(cfg: ZoidbergBotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("ZoidbergBot doctor");

  const root = await resolveZoidbergBotPackageRoot({
  ...
  let cfg: ZoidbergBotConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("zoidbergbot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("zoidbergbot config set gateway.mode local")}`,
    ];
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("zoidbergbot doctor --fix")}" to apply changes.`);
  }
