...
import { type ZoidbergBotConfig, readConfigFileSnapshot } from "../../config/config.js";
...
export async function requireValidConfig(
...
): Promise<ZoidbergBotConfig | null> {
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("zoidbergbot doctor")}.`);
    runtime.exit(1);
    return null;
  }
