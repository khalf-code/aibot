...
const requireAgentDir = () => {
  const agentDir = process.env.ZOIDBERGBOT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("ZOIDBERGBOT_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  const previousAgentDir = process.env.ZOIDBERGBOT_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.ZOIDBERGBOT_AGENT_DIR;
    } else {
      process.env.ZOIDBERGBOT_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.ZOIDBERGBOT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under ZOIDBERGBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-oauth-"));
    process.env.ZOIDBERGBOT_STATE_DIR = tempStateDir;
    process.env.ZOIDBERGBOT_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ZOIDBERGBOT_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ZOIDBERGBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-minimax-"));
    process.env.ZOIDBERGBOT_STATE_DIR = tempStateDir;
    process.env.ZOIDBERGBOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ZOIDBERGBOT_AGENT_DIR;
