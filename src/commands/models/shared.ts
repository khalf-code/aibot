...
export async function updateConfig(
  mutator: (cfg: ZoidbergBotConfig) => ZoidbergBotConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ZoidbergBotConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ZoidbergBotConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("zoidbergbot agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
