...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveZoidbergBotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("zoidbergbot models auth setup-token")}\` or \`${formatCliCommand("zoidbergbot configure")}\`.`
          : `Run \`${formatCliCommand("zoidbergbot configure")}\` or set an API key env var.`;
