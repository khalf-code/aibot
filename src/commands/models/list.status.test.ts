...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveZoidbergBotAgentDir: vi.fn().mockReturnValue("/tmp/zoidbergbot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/zoidbergbot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/zoidbergbot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveZoidbergBotAgentDir: mocks.resolveZoidbergBotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveZoidbergBotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/zoidbergbot-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/zoidbergbot-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/zoidbergbot-agent-custom");
