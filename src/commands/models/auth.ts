...
import { readConfigFileSnapshot, type ZoidbergBotConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: ZoidbergBotConfig, model: string): ZoidbergBotConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("zoidbergbot plugins install")}\`.`,
    );
  }
