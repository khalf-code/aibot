...
function resolveGroqApiKey(): string | undefined {
  const envDirect = process.env.GROQ_API_KEY?.trim();
  if (envDirect) {
    return envDirect;
  }

  const envResolved = resolveEnvApiKey("groq");
  if (envResolved?.apiKey) {
    return envResolved.apiKey;
  }

  const cfg = loadConfig();
  const key = getCustomProviderApiKey(cfg, "groq");
  if (key) {
    return key;
  }

  const store = ensureAuthProfileStore();
  const apiProfile = listProfilesForProvider(store, "groq").find((id) => {
    const cred = store.profiles[id];
    return cred?.type === "api_key" || cred?.type === "token";
  });
  if (!apiProfile) {
