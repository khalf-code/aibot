...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveZoidbergBotPackageRoot } = await import("./zoidbergbot-root.js");
    ...
    vi.mocked(resolveZoidbergBotPackageRoot).mockResolvedValue("/opt/zoidbergbot");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/zoidbergbot",
      installKind: "package",
      packageManager: "npm",
