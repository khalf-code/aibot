...
describe("ensureMoltbotCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling moltbot exists", async () => {
    ...
    try {
      ...
      const originalFlag = process.env.MOLTBOT_PATH_BOOTSTRAPPED;
      ...
      try {
        ...
        if (originalFlag === undefined) delete process.env.MOLTBOT_PATH_BOOTSTRAPPED;
        else process.env.MOLTBOT_PATH_BOOTSTRAPPED = originalFlag;
      }
    } finally {
