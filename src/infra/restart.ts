...
export function triggerZoidbergBotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ZOIDBERGBOT_SYSTEMD_UNIT,
        process.env.ZOIDBERGBOT_PROFILE,
      );
  ...
  const label =
    process.env.ZOIDBERGBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ZOIDBERGBOT_PROFILE);
