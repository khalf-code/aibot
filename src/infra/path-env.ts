...
type EnsureZoidbergBotPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureZoidbergBotPathOpts): string[] {
  ...
  // Bundled macOS app: `zoidbergbot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "zoidbergbot");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/zoidbergbot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "zoidbergbot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `zoidbergbot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureZoidbergBotCliOnPath(opts: EnsureZoidbergBotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ZOIDBERGBOT_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.ZOIDBERGBOT_PATH_BOOTSTRAPPED = "1";
