...
function resolveActiveHoursTimezone(cfg: ZoidbergBotConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ZoidbergBotConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ZoidbergBotConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: ZoidbergBotConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: ZoidbergBotConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ZoidbergBotConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ZoidbergBotConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ZoidbergBotConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
