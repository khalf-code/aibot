...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "zoidbergbot-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".zoidbergbot", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<ZoidbergBotConfig> = {},
): ZoidbergBotConfig {
  const base: ZoidbergBotConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "zoidbergbot"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".zoidbergbot", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
