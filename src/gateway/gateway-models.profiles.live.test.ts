...
import type { ZoidbergBotConfig, ModelProviderConfig } from "../config/types.js";
import { resolveZoidbergBotAgentDir } from "../agents/agent-paths.js";
...
import { ensureZoidbergBotModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ZOIDBERGBOT_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ZOIDBERGBOT_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ZOIDBERGBOT_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: ZoidbergBotConfig;
  agentDir: string;
}): ZoidbergBotConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<ZoidbergBotConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.ZOIDBERGBOT_AGENT_DIR,
  ...
  const hostAgentDir = resolveZoidbergBotAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-live-state-"));
  ...
  process.env.ZOIDBERGBOT_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.zoidbergbot-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveZoidbergBotAgentDir();
  const sanitizedCfg: ZoidbergBotConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "ZoidbergBot live tool probe (local, safe): " +
    ...
    process.env.ZOIDBERGBOT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ZOIDBERGBOT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureZoidbergBotModelsJson(cfg);
      ...
      const rawModels = process.env.ZOIDBERGBOT_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.zoidbergbot-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
