...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.ZOIDBERGBOT_SKIP_CHANNELS;
    prevSkipGmail = process.env.ZOIDBERGBOT_SKIP_GMAIL_WATCHER;
    process.env.ZOIDBERGBOT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.ZOIDBERGBOT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ZOIDBERGBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ZOIDBERGBOT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
