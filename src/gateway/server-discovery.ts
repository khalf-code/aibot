...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/zoidbergbot/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (ZoidbergBot)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.ZOIDBERGBOT_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "zoidbergbot");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.ZOIDBERGBOT_TAILNET_DNS?.trim();
