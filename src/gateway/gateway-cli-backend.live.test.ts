...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    if (!cliCommand) {
      throw new Error(`MOLTBOT_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`);
    }
    const baseCliArgs =
    ...
    try {
      ...
      if (previous.configPath === undefined) delete process.env.MOLTBOT_CONFIG_PATH;
      else process.env.MOLTBOT_CONFIG_PATH = previous.configPath;
      if (previous.token === undefined) delete process.env.MOLTBOT_GATEWAY_TOKEN;
      else process.env.MOLTBOT_GATEWAY_TOKEN = previous.token;
      if (previous.skipChannels === undefined) delete process.env.MOLTBOT_SKIP_CHANNELS;
      else process.env.MOLTBOT_SKIP_CHANNELS = previous.skipChannels;
      if (previous.skipGmail === undefined) delete process.env.MOLTBOT_SKIP_GMAIL_WATCHER;
      else process.env.MOLTBOT_SKIP_GMAIL_WATCHER = previous.skipGmail;
      if (previous.skipCron === undefined) delete process.env.MOLTBOT_SKIP_CRON;
      else process.env.MOLTBOT_SKIP_CRON = previous.skipCron;
      if (previous.skipCanvas === undefined) delete process.env.MOLTBOT_SKIP_CANVAS_HOST;
      else process.env.MOLTBOT_SKIP_CANVAS_HOST = previous.skipCanvas;
