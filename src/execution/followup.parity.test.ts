...
describe("Followup runner migration feature flag", () => {
  it("should be disabled by default", () => {
    const config = createMinimalConfig();
    expect(useNewExecutionLayer(config, "followup")).toBe(false);
  });

  it("should be enabled when execution.useNewLayer.followup is true", () => {
    const config = createMinimalConfig({
      execution: { useNewLayer: { followup: true } },
    });
    expect(useNewExecutionLayer(config, "followup")).toBe(true);
  });

  it("should respect global kill switch", () => {
    const config = createMinimalConfig({
      execution: { enabled: false, useNewLayer: { followup: true } },
    });
    expect(useNewExecutionLayer(config, "followup")).toBe(false);
  });
  ...
  it("should be independent from auto-reply and CLI flags", () => {
    const config = createMinimalConfig({
      execution: { useNewLayer: { cli: true, autoReply: true, followup: false } },
    });
    expect(useNewExecutionLayer(config, "followup")).toBe(false);
