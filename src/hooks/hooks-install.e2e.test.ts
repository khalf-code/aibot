...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.ZOIDBERGBOT_STATE_DIR;
    prevBundledDir = process.env.ZOIDBERGBOT_BUNDLED_HOOKS_DIR;
    process.env.ZOIDBERGBOT_STATE_DIR = path.join(baseDir, "state");
    process.env.ZOIDBERGBOT_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.ZOIDBERGBOT_BUNDLED_HOOKS_DIR;
    } else {
      process.env.ZOIDBERGBOT_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
      ...
      JSON.stringify(
        {
          ...
          version: "0.0.0",
          zoidbergbot: { hooks: ["./hooks/hello-hook"] },
        },
        null,
        2,
      ),
