...
`;
    ...
    expect(parsed.zoidbergbot.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.zoidbergbot.emoji).toBe("📝");
    expect(parsed.zoidbergbot.events).toEqual(["command"]);
    expect(parsed.zoidbergbot.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.zoidbergbot.install[0].kind).toBe("bundled");
  });
...
describe("resolveZoidbergBotMetadata", () => {
  it("extracts zoidbergbot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveZoidbergBotMetadata(frontmatter);
  ...
  it("returns undefined when zoidbergbot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        zoidbergbot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with ZoidbergBot" },
            { id: "npm", kind: "npm", package: "@zoidbergbot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@zoidbergbot/hook");
  });
    ...
    "zoidbergbot":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with ZoidbergBot" }],
      },
  }
---
...
`;
    ...
    const zoidbergbot = resolveZoidbergBotMetadata(frontmatter);
    expect(zoidbergbot).toBeDefined();
    expect(zoidbergbot?.emoji).toBe("💾");
    expect(zoidbergbot?.events).toEqual(["command:new"]);
    expect(zoidbergbot?.requires?.config).toEqual(["workspace.dir"]);
    expect(zoidbergbot?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const zoidbergbot = resolveZoidbergBotMetadata(frontmatter);
    expect(zoidbergbot?.emoji).toBe("disk");
    expect(zoidbergbot?.events).toEqual(["command:new"]);
  });
});
