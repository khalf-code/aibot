...
homepage: https://docs.zoidbergbot.ai/hooks#command-logger
metadata:
  {
    "zoidbergbot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.zoidbergbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.zoidbergbot/logs/commands.log ~/.zoidbergbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/zoidbergbot`:
   ```
   /home/username/.zoidbergbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
