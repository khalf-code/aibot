...
This directory contains hooks that ship with ZoidbergBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.zoidbergbot.ai/hooks/soul-evil
...
Hooks can be configured in `~/.zoidbergbot/zoidbergbot.json`:
...
2. Restart gateway: `pkill -9 -f 'zoidbergbot.*gateway' && pnpm zoidbergbot gateway`
3. Enable the hook: `zoidbergbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.zoidbergbot.ai/hooks
