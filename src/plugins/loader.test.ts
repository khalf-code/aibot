...
import { loadZoidbergBotPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.ZOIDBERGBOT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `zoidbergbot-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "zoidbergbot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ZOIDBERGBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadZoidbergBotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.ZOIDBERGBOT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadZoidbergBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadZoidbergBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
