...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ZoidbergBotPluginDefinition;
  register?: ZoidbergBotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ZoidbergBotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ZoidbergBotPluginDefinition;
...
export function loadZoidbergBotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverZoidbergBotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "zoidbergbot/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ZoidbergBotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as ZoidbergBotPluginModule;
    } catch (err) {
