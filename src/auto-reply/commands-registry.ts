...
import type { ZoidbergBotConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: ZoidbergBotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ZoidbergBotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ZoidbergBotConfig),
...
export function getCommandDetection(_cfg?: ZoidbergBotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ZoidbergBotConfig) {
