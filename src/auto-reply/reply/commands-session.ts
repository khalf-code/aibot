...
import { scheduleGatewaySigusr1Restart, triggerZoidbergBotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting ZoidbergBot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerZoidbergBotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting ZoidbergBot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
