...
const defaultTools = createZoidbergBotCodingTools();

describe("createZoidbergBotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createZoidbergBotCodingTools();
    ...
    try {
      ...
      const contents = "Hello from zoidbergbot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "zoidbergbot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "zoidbergbot-workspace"),
    ...
    const tools = createZoidbergBotCodingTools({ sandbox });
