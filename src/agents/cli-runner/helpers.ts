...
/**
 * Cleanup suspended ZoidbergBot CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: ZoidbergBotConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-cli-images-"));
  const paths: string[] = [];
