...
import { __testing, createZoidbergBotCodingTools } from "./pi-tools.js";
...
describe("createZoidbergBotCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createZoidbergBotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "zoidbergbot-outside.txt");
