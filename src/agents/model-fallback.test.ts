...
describe("runWithModelFallback", () => {
  ...
  it("only uses configured fallbacks, not hardcoded defaults (#5790)", async () => {
    // User explicitly configured only Google models — no Anthropic in their config.
    // The fallback logic should only use configured models, not add DEFAULT_PROVIDER.
    // Note: The root cause of #5790 was in resolveModelTarget (models/shared.ts),
    // but this test verifies the fallback loop behavior is also correct.
    const cfg = {
      agents: {
        defaults: {
          model: {
