...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "zoidbergbot.sandbox=1");
  args.push("--label", `zoidbergbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `zoidbergbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `zoidbergbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("zoidbergbot.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`zoidbergbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`zoidbergbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("zoidbergbot sandbox recreate --all");
}
