...
import type { ZoidbergBotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ZoidbergBotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: ZoidbergBotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ZoidbergBotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ZoidbergBotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ZoidbergBotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as ZoidbergBotConfig)),
    ),
  };
}
