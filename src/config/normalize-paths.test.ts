  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.zoidbergbot/logs/zoidbergbot.log" },
        hooks: {
          path: "~/.zoidbergbot/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.zoidbergbot/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".zoidbergbot", "logs", "zoidbergbot.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".zoidbergbot", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".zoidbergbot", "telegram.token"),
      );
