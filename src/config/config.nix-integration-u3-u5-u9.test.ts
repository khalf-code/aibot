...
describe("Nix integration (U3, U5, U9)", () => {
  describe("U3: isNixMode env var detection", () => {
    it("isNixMode is false when MOLTBOT_NIX_MODE is not set", async () => {
      await withEnvOverride({ MOLTBOT_NIX_MODE: undefined }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is false when MOLTBOT_NIX_MODE is empty", async () => {
      await withEnvOverride({ MOLTBOT_NIX_MODE: "" }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is false when MOLTBOT_NIX_MODE is not '1'", async () => {
      await withEnvOverride({ MOLTBOT_NIX_MODE: "true" }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is true when MOLTBOT_NIX_MODE=1", async () => {
      await withEnvOverride({ MOLTBOT_NIX_MODE: "1" }, async () => {
        const { isNixMode } = await import("./config.js");
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.moltbot when env not set", async () => {
      await withEnvOverride(
        { MOLTBOT_STATE_DIR: undefined, MOLTBOT_STATE_DIR: undefined },
        async () => {
          const { STATE_DIR } = await import("./config.js");
          expect(STATE_DIR).toMatch(/\.moltbot$/);
        },
      );
    });

    it("STATE_DIR respects MOLTBOT_STATE_DIR override", async () => {
      await withEnvOverride(
        { MOLTBOT_STATE_DIR: undefined, MOLTBOT_STATE_DIR: "/custom/state/dir" },
        async () => {
          const { STATE_DIR } = await import("./config.js");
    ...
    it("STATE_DIR prefers MOLTBOT_STATE_DIR over legacy override", async () => {
      await withEnvOverride(
        { MOLTBOT_STATE_DIR: "/custom/new", MOLTBOT_STATE_DIR: "/custom/legacy" },
        async () => {
          const { STATE_DIR } = await import("./config.js");
    ...
    it("CONFIG_PATH defaults to ~/.moltbot/moltbot.json when env not set", async () => {
      await withEnvOverride(
        {
        ...
        async () => {
          ...
          expect(CONFIG_PATH).toMatch(/\.moltbot[\\/]moltbot\.json$/);
        },
      );
    });

    it("CONFIG_PATH respects MOLTBOT_CONFIG_PATH override", async () => {
      await withEnvOverride(
        { MOLTBOT_CONFIG_PATH: undefined, MOLTBOT_CONFIG_PATH: "/nix/store/abc/moltbot.json" },
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
    ...
    it("CONFIG_PATH expands ~ in MOLTBOT_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        await withEnvOverride(
          { MOLTBOT_CONFIG_PATH: undefined, MOLTBOT_CONFIG_PATH: "~/.moltbot/custom.json" },
          async () => {
            const { CONFIG_PATH } = await import("./config.js");
            expect(CONFIG_PATH).toBe(path.join(home, ".moltbot", "custom.json"));
          },
        );
      });
    });
  ...
  describe("U6: gateway port resolution", () => {
    ...
    it("falls back to config when env is invalid", async () => {
      await withEnvOverride({ MOLTBOT_GATEWAY_PORT: "nope" }, async () => {
        const { resolveGatewayPort } = await import("./config.js");
