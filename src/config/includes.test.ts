...
const ETC_ZOIDBERGBOT_DIR = path.join(ROOT_DIR, "etc", "zoidbergbot");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "zoidbergbot.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcZoidbergBotPath(...parts: string[]) {
  return path.join(ETC_ZOIDBERGBOT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcZoidbergBotPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "zoidbergbot.json"))).toEqual({
      shared: true,
    });
  });
});
