...
import type { ZoidbergBotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareZoidbergBotVersions } from "./version.js";
...
function coerceConfig(value: unknown): ZoidbergBotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ZoidbergBotConfig;
}
...
function stampConfigVersion(cfg: ZoidbergBotConfig): ZoidbergBotConfig {
...
function warnIfConfigFromFuture(
  ...
  const cmp = compareZoidbergBotVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer ZoidbergBot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: ZoidbergBotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ZoidbergBotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as ZoidbergBotConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ZoidbergBotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ZoidbergBotConfig) {
    clearConfigCache();
...
// module scope. `ZOIDBERGBOT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ZOIDBERGBOT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ZOIDBERGBOT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): ZoidbergBotConfig {
...
export async function writeConfigFile(cfg: ZoidbergBotConfig): Promise<void> {
  clearConfigCache();
