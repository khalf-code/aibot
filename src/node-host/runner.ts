...
const execHostEnforced = process.env.ZOIDBERGBOT_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.ZOIDBERGBOT_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureZoidbergBotCliOnPath({ pathEnv: process.env.PATH ?? "" });
