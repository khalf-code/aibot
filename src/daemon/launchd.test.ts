...
async function withLaunchctlStub(
  ...
  const originalLogPath = process.env.MOLTBOT_TEST_LAUNCHCTL_LOG;
  const originalListOutput = process.env.MOLTBOT_TEST_LAUNCHCTL_LIST_OUTPUT;
  ...
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const logPath = process.env.MOLTBOT_TEST_LAUNCHCTL_LOG;",
        "if (logPath) {",
        '  fs.appendFileSync(logPath, JSON.stringify(args) + "\\n", "utf8");',
        "}",
        'if (args[0] === "list") {',
        '  const output = process.env.MOLTBOT_TEST_LAUNCHCTL_LIST_OUTPUT || "";',
    ...
    process.env.MOLTBOT_TEST_LAUNCHCTL_LOG = logPath;
    process.env.MOLTBOT_TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    ...
    if (originalLogPath === undefined) {
      delete process.env.MOLTBOT_TEST_LAUNCHCTL_LOG;
    } else {
      process.env.MOLTBOT_TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env.MOLTBOT_TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env.MOLTBOT_TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
