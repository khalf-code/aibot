...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.ZOIDBERGBOT_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.ZOIDBERGBOT_STATE_DIR;
  const configPath = env.ZOIDBERGBOT_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    ZOIDBERGBOT_PROFILE: profile,
    ZOIDBERGBOT_STATE_DIR: stateDir,
    ZOIDBERGBOT_CONFIG_PATH: configPath,
    ZOIDBERGBOT_GATEWAY_PORT: String(port),
    ZOIDBERGBOT_GATEWAY_TOKEN: token,
    ZOIDBERGBOT_LAUNCHD_LABEL: resolvedLaunchdLabel,
    ZOIDBERGBOT_SYSTEMD_UNIT: systemdUnit,
    ZOIDBERGBOT_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    ZOIDBERGBOT_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    ZOIDBERGBOT_STATE_DIR: stateDir,
    ZOIDBERGBOT_CONFIG_PATH: configPath,
    ZOIDBERGBOT_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    ZOIDBERGBOT_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    ZOIDBERGBOT_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    ZOIDBERGBOT_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    ZOIDBERGBOT_LOG_PREFIX: "node",
    ZOIDBERGBOT_SERVICE_MARKER: NODE_SERVICE_MARKER,
    ZOIDBERGBOT_SERVICE_KIND: NODE_SERVICE_KIND,
    ZOIDBERGBOT_SERVICE_VERSION: VERSION,
  };
}
