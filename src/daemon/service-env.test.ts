...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ZOIDBERGBOT_GATEWAY_PORT).toBe("18789");
    expect(env.ZOIDBERGBOT_GATEWAY_TOKEN).toBe("secret");
    expect(env.ZOIDBERGBOT_SERVICE_MARKER).toBe("zoidbergbot");
    expect(env.ZOIDBERGBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ZOIDBERGBOT_SERVICE_VERSION).toBe("string");
    expect(env.ZOIDBERGBOT_SYSTEMD_UNIT).toBe("zoidbergbot-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ZOIDBERGBOT_LAUNCHD_LABEL).toBe("ai.zoidbergbot.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ZOIDBERGBOT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ZOIDBERGBOT_SYSTEMD_UNIT).toBe("zoidbergbot-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ZOIDBERGBOT_LAUNCHD_LABEL).toBe("ai.zoidbergbot.work");
    }
  });
});
