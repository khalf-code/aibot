...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `zoidbergbot --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.ZOIDBERGBOT_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.ZOIDBERGBOT_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[zoidbergbot] Gateway daemon failed:",
