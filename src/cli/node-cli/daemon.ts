...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("zoidbergbot node install"),
    formatCliCommand("zoidbergbot node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("zoidbergbot node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ZOIDBERGBOT_LOG_PREFIX: baseEnv.ZOIDBERGBOT_LOG_PREFIX ?? "node",
