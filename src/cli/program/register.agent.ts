...
${formatHelpExamples([
  ['zoidbergbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['zoidbergbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['zoidbergbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.zoidbergbot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.zoidbergbot.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'zoidbergbot agents set-identity --agent main --name "ZoidbergBot" --emoji "🦞"',
    "Set name + emoji.",
  ],
  [
  ...
  [
    "zoidbergbot agents set-identity --identity-file ~/.zoidbergbot/workspace/IDENTITY.md --agent main",
