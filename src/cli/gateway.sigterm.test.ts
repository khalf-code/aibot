...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zoidbergbot-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      ZOIDBERGBOT_NO_RESPAWN: "1",
      ZOIDBERGBOT_STATE_DIR: stateDir,
      ZOIDBERGBOT_SKIP_CHANNELS: "1",
      ZOIDBERGBOT_SKIP_GMAIL_WATCHER: "1",
      ZOIDBERGBOT_SKIP_CRON: "1",
      ZOIDBERGBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
      ZOIDBERGBOT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "zoidbergbot-entry-bootstrap.mjs");
