...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.zoidbergbot.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + ZoidbergBot hooks")
    ...
    .option("--hook-url <url>", "ZoidbergBot hook URL")
    .option("--hook-token <token>", "ZoidbergBot hook token")
