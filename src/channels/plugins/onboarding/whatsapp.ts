...
function setWhatsAppDmPolicy(cfg: ZoidbergBotConfig, dmPolicy: DmPolicy): ZoidbergBotConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: ZoidbergBotConfig, allowFrom?: string[]): ZoidbergBotConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: ZoidbergBotConfig, selfChatMode: boolean): ZoidbergBotConfig {
...
async function detectWhatsAppLinked(cfg: ZoidbergBotConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so ZoidbergBot can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for ZoidbergBot" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("zoidbergbot channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
