...
function setSlackDmPolicy(cfg: ZoidbergBotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "ZoidbergBot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for ZoidbergBot`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: ZoidbergBotConfig, allowFrom: string[]): ZoidbergBotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
