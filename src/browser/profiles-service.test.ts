...
vi.mock("./chrome.js", () => ({
  resolveZoidbergBotUserDataDir: vi.fn(() => "/tmp/zoidbergbot-test/zoidbergbot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveZoidbergBotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "zoidbergbot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveZoidbergBotUserDataDir).mockReturnValue(userDataDir);
