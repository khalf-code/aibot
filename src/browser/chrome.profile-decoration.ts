...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".zoidbergbot-profile-decorated");
}
 ...
 */
export function decorateZoidbergBotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ZOIDBERGBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ZOIDBERGBOT_BROWSER_COLOR).toUpperCase();
