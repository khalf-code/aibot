import type { BrowserConfig, BrowserProfileConfig, ZoidbergBotConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_ZOIDBERGBOT_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_ZOIDBERGBOT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "zoidbergbot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ZOIDBERGBOT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ZOIDBERGBOT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an ZoidbergBot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openclaw/zoidbergbot CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ZoidbergBotConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ZOIDBERGBOT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "zoidbergbot";
