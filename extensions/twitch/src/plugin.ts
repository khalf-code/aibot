/**
 * Twitch channel plugin for ZoidbergBot.
 *
...
import { buildChannelConfigSchema } from "zoidbergbot/plugin-sdk";
import type {
 ...
 * for ZoidbergBot. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ZoidbergBotConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ZoidbergBotConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ZoidbergBotConfig): boolean => {
