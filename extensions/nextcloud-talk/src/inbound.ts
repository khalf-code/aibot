...
export async function handleNextcloudTalkInbound(params: {
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as ZoidbergBotConfig,
    surface: CHANNEL_ID,
  });
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as ZoidbergBotConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ZoidbergBotConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as ZoidbergBotConfig);
