...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ZOIDBERGBOT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zoidbergbot-nostr-"));
  process.env.ZOIDBERGBOT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.ZOIDBERGBOT_STATE_DIR?.trim() || env.ZOIDBERGBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".zoidbergbot");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ZOIDBERGBOT_STATE_DIR = previous;
    }
