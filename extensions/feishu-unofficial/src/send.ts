...
async function readLocalImageBuffer(
  localPath: string,
...
async function uploadFeishuImage(params: {
  ...
  try {
    ...
    if (data.code && data.code !== 0) {
      throw new Error(
        `Feishu image upload failed: ${data.msg ?? "unknown error"} (code: ${data.code})`,
      );
    }
  ...
  const client = createFeishuClientFromConfig(cfg) as unknown as {
    im?: {
      v1?: {
        file?: {
          create: (payload: {
            data: { file_type: string; file_name: string; file: Buffer | fs.ReadStream | Readable };
...
function nextImageMatch(text: string, fromIndex: number): ImageMatch | null {
  ...
  const candidates = [
    linkedMatch ? { kind: "linked", index: linkedMatch.index, match: linkedMatch } : null,
