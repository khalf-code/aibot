...
metadata: { "zoidbergbot": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == ZoidbergBot `sessions_spawn`
- **File I/O** == ZoidbergBot `read`/`write`
- **Remote fetch** == ZoidbergBot `web_fetch` (or `exec` with curl when POST is required)
