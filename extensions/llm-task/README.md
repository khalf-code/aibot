...
`zoidbergbot.invoke --each`) without adding new ZoidbergBot code per workflow.
...
This extension depends on ZoidbergBot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ZoidbergBot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.zoidbergbot/extensions` as a standalone plugin directory.
