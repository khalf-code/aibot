...
${userContent}`;
        ...
        log?.info(
          `[qqbot:${account.accountId}] ctxPayload: From=${fromAddress}, To=${toAddress}, SessionKey=${route.sessionKey}, AccountId=${route.accountId}, ChatType=${isGroup ? "group" : "direct"}, SenderId=${event.senderId}, MessageSid=${event.messageId}, BodyLen=${body?.length ?? 0}`,
        );
        ...
        try {
          ...
          // 调用 dispatchReply
          log?.info(
            `[qqbot:${account.accountId}] dispatchReply: agentId=${route.agentId}, prefix=${messagesConfig.responsePrefix ?? "(none)"}`,
          );
            ...
            pluginRuntime.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
              ...
              dispatcherOptions: {
                ...
                deliver: async (
                  ...
                  log?.info(
                    `[qqbot:${account.accountId}] deliver(${info.kind}): textLen=${payload.text?.length ?? 0}, mediaUrls=${payload.mediaUrls?.length ?? 0}, mediaUrl=${payload.mediaUrl ? "yes" : "no"}`,
                  );
                  if (payload.text) {
                    log?.info(
                      `[qqbot:${account.accountId}] text preview: ${payload.text.slice(0, 150).replace(/\n/g, "\\n")}...`,
                    );
                  }
                    ...
                    if (localPath && imageServerBaseUrl) {
                      try {
                        ...
                        if (savedUrl) {
                          ...
                          log?.info(
                            `[qqbot:${account.accountId}] Saved media to server: ${localPath}`,
                          );
                        } else {
                          log?.error(
                            `[qqbot:${account.accountId}] Failed to save media (not found or not image): ${localPath}`,
                          );
                        }
                      } catch (err) {
                    ...
                    if (localPath && imageServerBaseUrl) {
                      ...
                      try {
                        ...
                        if (savedUrl) {
                          imageUrls.push(savedUrl);
                          log?.info(
                            `[qqbot:${account.accountId}] Saved local image to server: ${localPath}`,
                          );
                        } else {
                          log?.error(
                            `[qqbot:${account.accountId}] Failed to save local image (not found or not image): ${localPath}`,
                          );
                        }
                      } catch (err) {
                        log?.error(
                          `[qqbot:${account.accountId}] Failed to save local image: ${err}`,
                        );
                      }
                    }
                    // 从文本中移除 MEDIA: 行
                    ...
                    if (localPath && imageServerBaseUrl) {
                      ...
                      try {
                        ...
                        if (savedUrl) {
                          imageUrls.push(savedUrl);
                          log?.info(
                            `[qqbot:${account.accountId}] Saved local path image to server: ${localPath}`,
                          );
                        } else {
                          log?.error(
                            `[qqbot:${account.accountId}] Local path not found or not image: ${localPath}`,
                          );
                        }
                      } catch (err) {
                        log?.error(
                          `[qqbot:${account.accountId}] Failed to save local path image: ${err}`,
                        );
                      }
                    }
                    // 从文本中移除本地路径
                  ...
                  const base64ImageRegex =
                    /!\[([^\]]*)\]\((data:image\/[^;]+;base64,[A-Za-z0-9+/=]+)\)|(?<![(\[])(data:image\/[^;]+;base64,[A-Za-z0-9+/=]+)/gi;
                  ...
                  for (const match of base64Matches) {
                    ...
                    if (dataUrl && imageServerBaseUrl) {
                      ...
                      try {
                        ...
                        imageUrls.push(savedUrl);
                        log?.info(
                          `[qqbot:${account.accountId}] Saved base64 image to local server`,
                        );
                      } catch (err) {
                        log?.error(
                          `[qqbot:${account.accountId}] Failed to save base64 image: ${err}`,
                        );
                      }
                    }
                    // 从文本中移除 base64
                  ...
                  const imageUrlRegex =
                    /!\[([^\]]*)\]\((https?:\/\/[^\s)]+\.(?:png|jpg|jpeg|gif|webp)(?:\?[^\s)]*)?)\)|(?<![(\[])(https?:\/\/[^\s)]+\.(?:png|jpg|jpeg|gif|webp)(?:\?[^\s]*)?)/gi;
                  ...
                  try {
                    ...
                    for (const imageUrl of imageUrls) {
                      try {
                        await sendWithTokenRetry(async (token) => {
                          if (event.type === "c2c") {
                            log?.info(
                              `[qqbot:${account.accountId}] sendC2CImage -> ${event.senderId}`,
                            );
                            ...
                            log?.info(
                              `[qqbot:${account.accountId}] sendGroupImage -> ${event.groupOpenid}`,
                            );
                    ...
                    if (replyText.trim()) {
                      await sendWithTokenRetry(async (token) => {
                        if (event.type === "c2c") {
                          log?.info(
                            `[qqbot:${account.accountId}] sendC2CText -> ${event.senderId}, len=${replyText.length}`,
                          );
                          await sendC2CMessage(token, event.senderId, replyText, event.messageId);
                        } else if (event.type === "group" && event.groupOpenid) {
                          log?.info(
                            `[qqbot:${account.accountId}] sendGroupText -> ${event.groupOpenid}, len=${replyText.length}`,
                          );
                          ...
                          log?.info(
                            `[qqbot:${account.accountId}] sendChannelText -> ${event.channelId}, len=${replyText.length}`,
                          );
          ...
          try {
            ...
            if (!hasResponse) {
              ...
              await sendErrorMessage(
                "QQ Channel 收到了你的请求，已经转交给了Openclaw，正在处理中...",
              );
            }
          }
        } catch (err) {
