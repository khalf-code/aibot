...
function buildStreamPlaceholderReply(streamId: string): {
  msgtype: "stream";
...
export async function handleWecomWebhookRequest(
  ...
  logger.debug(
    `incoming ${req.method} request on ${path} (timestamp=${timestamp}, nonce=${nonce})`,
  );
  ...
  const record =
    body.value && typeof body.value === "object" ? (body.value as Record<string, unknown>) : null;
  ...
  if (msgtype === "event") {
    const eventtype = String(
      (msg as { event?: { eventtype?: string } }).event?.eventtype ?? "",
    ).toLowerCase();
  ...
  const state = streams.get(streamId);
  const initialReply =
    state && (state.content.trim() || state.error)
      ? buildStreamReplyFromState(state)
