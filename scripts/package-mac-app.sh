...
# Build and bundle ZoidbergBot into a minimal .app we can open.
# Outputs to dist/ZoidbergBot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ZoidbergBot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="ZoidbergBot"
BUNDLE_ID="${BUNDLE_ID:-ai.zoidbergbot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/zoidbergbot/zoidbergbot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ZoidbergBot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ZoidbergBotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ZoidbergBotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ZoidbergBot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ZoidbergBot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ZoidbergBot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ZoidbergBot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ZoidbergBot/Resources/ZoidbergBot.icns" "$APP_ROOT/Contents/Resources/ZoidbergBot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ZoidbergBot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ZoidbergBotKit resources"
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ZoidbergBotKit_ZoidbergBotKit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ZoidbergBotKit_ZoidbergBotKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ZoidbergBotKit_ZoidbergBotKit.bundle"
else
  echo "WARN: ZoidbergBotKit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running ZoidbergBot"
killall -q ZoidbergBot 2>/dev/null || true
