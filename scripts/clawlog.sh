...
show_usage() {
    cat << EOF
clawlog - ZoidbergBot Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View ZoidbergBot logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    ZoidbergBot logs flow through the macOS unified log (subsystem: ai.zoidbergbot).
