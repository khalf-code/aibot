...
INSTALL_URL="${ZOIDBERGBOT_INSTALL_URL:-https://zoidbergbot.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ZOIDBERGBOT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ZOIDBERGBOT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="zoidbergbot"
PACKAGE_NAME="${ZOIDBERGBOT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ZOIDBERGBOT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ZOIDBERGBOT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ZOIDBERGBOT_INSTALL_LATEST_OUT:-}"
fi
