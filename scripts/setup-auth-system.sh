#!/bin/bash
# Setup ZoidbergBot Auth Management System
...
echo "=== ZoidbergBot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'zoidbergbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ZoidbergBot message: Send warning via ZoidbergBot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/zoidbergbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/zoidbergbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/zoidbergbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now zoidbergbot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status zoidbergbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
