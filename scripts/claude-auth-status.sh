...
# Checks both Claude Code and ZoidbergBot auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
ZOIDBERGBOT_AUTH="$HOME/.zoidbergbot/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    zoidbergbot models status --json 2>/dev/null || true
}
...
check_openclaw_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$ZOIDBERGBOT_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    if [ "$USE_JSON" -eq 1 ]; then
        ...
        openclaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ZOIDBERGBOT_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            zoidbergbot: {status: $bs, expires_at_ms: ($be | tonumber)},
...
echo ""
echo "ZoidbergBot Auth (~/.zoidbergbot/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$ZOIDBERGBOT_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$ZOIDBERGBOT_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'zoidbergbot doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active zoidbergbot >/dev/null 2>&1; then
    echo -e "ZoidbergBot service: ${GREEN}running${NC}"
else
    echo -e "ZoidbergBot service: ${RED}NOT running${NC}"
fi
