...
ZIP=${1:?"Usage: $0 ZoidbergBot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/zoidbergbot/zoidbergbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^ZoidbergBot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/zoidbergbot/zoidbergbot/releases/download/v${VERSION}/"}
