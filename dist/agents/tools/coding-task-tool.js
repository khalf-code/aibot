...
const CodingTaskToolSchema = Type.Object({
    task: Type.String({
        description: "A coding task to run via Claude Agent SDK (Claude Code-style; capabilities gated by tools.codingTask).",
    }),
});
...
async function coerceAsyncIterable(value) {
...
function resolveCodingTaskEnabled(cfg) {
...
export function createCodingTaskTool(opts) {
    return {
        label: "Coding",
        name: "coding_task",
        description: "Run a Claude Code-style coding task via the Claude Agent SDK (experimental; gated by tools.codingTask).",
        ...
        execute: async (_toolCallId, args) => {
            ...
            try {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: "Claude Agent SDK is not available. Install @anthropic-ai/claude-agent-sdk " +
            ...
            try {
                const stream = await coerceAsyncIterable(sdk.query({
                    prompt: task,
