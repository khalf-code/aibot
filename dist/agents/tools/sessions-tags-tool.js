...
import { createAgentToAgentPolicy, resolveInternalSessionKey, resolveMainSessionAlias, resolveSessionReference, } from "./sessions-helpers.js";
...
export function createSessionsTagsTool(opts) {
    return {
        label: "Session Tags",
        name: "sessions_tags",
        description: "Set/add/remove tags on a session for filtering/slicing in the Sessions UI. Default: current session.",
        ...
        execute: async (_toolCallId, args) => {
            ...
            const sessionKeyParam = readStringParam(params, "sessionKey")?.trim() || opts?.agentSessionKey?.trim();
            if (!sessionKeyParam) {
            ...
            if (requesterAgentId !== targetAgentId) {
                if (!a2aPolicy.enabled) {
                    return jsonResult({
                        ok: false,
                        status: "forbidden",
                        error: "Cross-agent access is disabled. Set tools.agentToAgent.enabled=true and allow both agent IDs.",
                    });
                }
            ...
            if (restrictToSpawned && !resolvedSession.resolvedViaSessionId) {
                ...
                const ok = (Array.isArray(sessions?.sessions) ? sessions.sessions : []).some((entry) => entry?.key === canonicalKey);
                if (!ok) {
                    return jsonResult({
                        ok: false,
                        status: "forbidden",
