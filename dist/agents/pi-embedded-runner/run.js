...
export async function runEmbeddedPiAgent(params) {
    ...
    return enqueueSession(() => enqueueGlobal(async () => {
        ...
        const emitManualCompactionEvent = (data) => {
        ...
        try {
            while (true) {
                ...
                const assistantErrorText = lastAssistant?.stopReason === "error" ? (lastAssistant.errorMessage ?? "").trim() : "";
                if (!aborted && assistantErrorText && isContextOverflowError(assistantErrorText)) {
                    ...
                    if (!isCompactionFailure && !overflowCompactionAttempted) {
                        log.warn(`context overflow detected (assistant error); attempting auto-compaction for ${provider}/${modelId}`);
                        overflowCompactionAttempted = true;
                        emitOverflowDiagnostics({
                            source: "assistant_error",
                            phase: "compaction_start",
                        ...
                        if (compactResult.compacted) {
                            log.info(`auto-compaction succeeded (assistant error) for ${provider}/${modelId}; retrying prompt`);
                            emitOverflowDiagnostics({
                                source: "assistant_error",
                                phase: "compaction_end",
                                errorText: assistantErrorText,
                        ...
                        log.warn(`auto-compaction failed (assistant error) for ${provider}/${modelId}: ${compactResult.reason ?? "nothing to compact"}`);
                        emitOverflowDiagnostics({
                            source: "assistant_error",
                            phase: "compaction_end",
                            errorText: assistantErrorText,
