...
export async function monitorSlackProvider(opts = {}) {
    ...
    runtime.log?.(`slack: init account=${account.accountId} mode=${slackMode} dm=${dmEnabled ? dmPolicy : "disabled"} groupPolicy=${groupPolicy} channels=${channelKeys.length}${channelWildcard ? "+*" : ""} allowFrom=${allowFromCount} requireMentionDefault=${slackCfg.requireMention ?? true}`);
    ...
    try {
        ...
        runtime.log?.(`slack: auth ok team=${teamId || "unknown"} botUser=${botUserId || "unknown"} app=${apiAppId || "unknown"}`);
    }
    catch {
