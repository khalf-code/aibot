 ...
 */
import { createDecision, getDecision, respondToDecision, updateDecisionSlackInfo, } from "../../infra/decisions/store.js";
...
function parseDecisionButtonValue(value) {
...
export function createSlackDecisionManager(params) {
    ...
    const postDecision = async (createParams) => {
        ...
        const decision = createDecision({
            ...createParams,
            timeoutMinutes: createParams.timeoutMinutes ?? Math.floor(resolvedConfig.decisionTimeoutMs / 60000),
        });
    ...
    const handleDecisionResponse = async (decision, optionId, optionValue, userId, userName) => {
        // Record the response
        ...
        if (decision.slackChannel && decision.slackMessageTs) {
            ...
            await bridge.updateMessage(decision.slackChannel, decision.slackMessageTs, `Decision resolved: ${decision.title}`, resolvedBlocks);
        }
    ...
    const handleTextDecisionResponse = async (decision, textValue, userId, userName) => {
        const updated = respondToDecision({
            decisionId: decision.decisionId,
            textValue,
            respondedBy: { userId, userName },
    ...
    const registerActionHandlers = () => {
        ...
        appWithAction.action(new RegExp(`^${DECISION_BUTTON_ACTION_PREFIX}(?!text_)`), async (args) => {
            ...
            const userName = "name" in user ? user.name : "username" in user ? user.username : undefined;
            await handleDecisionResponse(decision, parsed.optionId, parsed.optionValue, user.id, userName);
        });
        // Handler for text decision button (opens modal)
        appWithAction.action(new RegExp(`^${DECISION_TEXT_ACTION_PREFIX}`), async (args) => {
            const { ack, body, action } = args;
            await ack();
            const buttonAction = action;
...
async function routeDecisionResponseToAgent(decision) {
    const sessionKey = decision.context.sessionKey;
    if (!sessionKey)
        return;
    const responseText = decision.response?.textValue ??
