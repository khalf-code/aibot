...
import { ErrorCodes, errorShape, formatValidationErrors, validateOverseerGoalCreateParams, validateOverseerGoalStatusParams, validateOverseerGoalUpdateParams, validateOverseerStatusParams, validateOverseerTickParams, validateOverseerWorkUpdateParams, } from "../protocol/index.js";
...
function updateGoalRollups(goal) {
...
function serializePlanNodeBase(node) {
    return {
        id: node.id,
...
function serializeGoalForStatus(goal) {
    return {
        goalId: goal.goalId,
        title: goal.title,
...
export const overseerHandlers = {
    "overseer.status": ({ params, respond }) => {
        if (!validateOverseerStatusParams(params)) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, `invalid overseer.status params: ${formatValidationErrors(validateOverseerStatusParams.errors)}`));
            return;
        }
    ...
    "overseer.goal.create": async ({ params, respond }) => {
        if (!validateOverseerGoalCreateParams(params)) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, `invalid overseer.goal.create params: ${formatValidationErrors(validateOverseerGoalCreateParams.errors)}`));
            return;
        }
        const request = params;
        ...
        const goal = {
            ...
            updatedAt: Date.now(),
            status: "active",
            priority: request.priority === "low" || request.priority === "high" || request.priority === "urgent"
                ? request.priority
                : "normal",
        ...
        if (shouldGenerate) {
            ...
            planGenerated = true;
            const safeIdleAfterMs = Number.isFinite(idleAfterMs) && idleAfterMs > 0 ? idleAfterMs : 15 * 60_000;
        ...
        const result = { goalId, planGenerated };
        respond(true, result, undefined);
    },
    "overseer.goal.status": ({ params, respond }) => {
        if (!validateOverseerGoalStatusParams(params)) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, `invalid overseer.goal.status params: ${formatValidationErrors(validateOverseerGoalStatusParams.errors)}`));
            return;
        }
        const request = params;
        const store = loadOverseerStoreFromDisk();
        const goal = store.goals?.[request.goalId];
        const assignments = Object.values(store.assignments ?? {}).filter((assignment) => assignment.goalId === request.goalId);
        const crystallizations = Object.values(store.crystallizations ?? {}).filter((crystallization) => crystallization.goalId === request.goalId);
    ...
    "overseer.goal.pause": async ({ params, respond }) => {
        if (!validateOverseerGoalStatusParams(params)) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, `invalid overseer.goal.pause params: ${formatValidationErrors(validateOverseerGoalStatusParams.errors)}`));
            return;
        }
        const request = params;
        await updateOverseerStore(async (store) => {
    ...
    "overseer.goal.resume": async ({ params, respond }) => {
        if (!validateOverseerGoalStatusParams(params)) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, `invalid overseer.goal.resume params: ${formatValidationErrors(validateOverseerGoalStatusParams.errors)}`));
            return;
        }
        const request = params;
        await updateOverseerStore(async (store) => {
    ...
    "overseer.goal.update": async ({ params, respond }) => {
        if (!validateOverseerGoalUpdateParams(params)) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, `invalid overseer.goal.update params: ${formatValidationErrors(validateOverseerGoalUpdateParams.errors)}`));
            return;
        }
        const request = params;
        await updateOverseerStore(async (store) => {
    ...
    "overseer.goal.cancel": async ({ params, respond }) => {
        if (!validateOverseerGoalStatusParams(params)) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, `invalid overseer.goal.cancel params: ${formatValidationErrors(validateOverseerGoalStatusParams.errors)}`));
            return;
        }
        const request = params;
        const now = Date.now();
    ...
    "overseer.work.update": async ({ params, respond }) => {
        if (!validateOverseerWorkUpdateParams(params)) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, `invalid overseer.work.update params: ${formatValidationErrors(validateOverseerWorkUpdateParams.errors)}`));
            return;
        }
        const request = params;
    ...
    "overseer.tick": async ({ params, respond, context }) => {
        if (!validateOverseerTickParams(params)) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, `invalid overseer.tick params: ${formatValidationErrors(validateOverseerTickParams.errors)}`));
            return;
        }
        const request = params;
