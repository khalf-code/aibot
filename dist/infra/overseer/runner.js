...
import { executeOverseerActions } from "./dispatcher.js";
import { createOverseerMonitor } from "./monitor.js";
import { generateOverseerPlan } from "./planner.js";
import { updateOverseerStore, loadOverseerStoreFromDisk } from "./store.js";
import { requestOverseerNow, setOverseerWakeHandler } from "./wake.js";
...
function resolveOverseerConfig(cfg = loadConfig()) {
    const safeDuration = (value, fallback) => {
        try {
    ...
    const minResendIntervalMs = safeDuration(cfg.overseer?.minResendInterval ?? DEFAULT_MIN_RESEND, 5 * 60_000);
    const backoffBaseMs = safeDuration(cfg.overseer?.backoff?.base ?? DEFAULT_BACKOFF_BASE, 2 * 60_000);
    ...
    const allowAgents = new Set(allowAgentsRaw
...
function findWorkNode(goal, workNodeId) {
...
function isAssignmentAllowed(cfg, assignment) {
    if (!cfg.allowCrossAgent) {
        const targetAgent = normalizeAgentId(assignment.agentId ?? resolveAgentIdFromSessionKey(assignment.sessionKey));
        return targetAgent === cfg.defaultAgentId;
    }
    if (cfg.allowAnyAgent)
        return true;
    const agentId = normalizeAgentId(assignment.agentId ?? resolveAgentIdFromSessionKey(assignment.sessionKey));
...
export function applyStructuredUpdate(params) {
    ...
    const status = params.update.status === "done" ||
...
export function reconcileOverseerState(params) {
    ...
    for (const assignment of assignments) {
        ...
        else if (telemetryEntry?.sessionUpdatedAt &&
        ...
        else if (telemetryEntry?.lastMessageFingerprint &&
        ...
        const lastActivity = assignment.lastObservedActivityAt ?? assignment.lastDispatchAt ?? assignment.updatedAt;
        const idleAfterMs = assignment.idleAfterMs ?? cfg.idleAfterMs;
        const wasStalled = assignment.status === "stalled";
        const isOverdue = (assignment.expectedNextUpdateAt && assignment.expectedNextUpdateAt <= now) ||
        ...
        if (assignment.status === "stalled") {
            ...
            const reuseDispatchId = actionType === "resend_last" &&
            ...
            assignment.expectedNextUpdateAt = resolveExpectedNextUpdateAt(now, idleAfterMs);
            const message = assignment.lastInstructionText ??
                buildNudgeMessage({
                    goalId: assignment.goalId,
                    workNodeId: assignment.workNodeId,
...
async function maybeApplyPlanner(store, now) {
    const stalledAssignments = Object.values(store.assignments ?? {}).filter((assignment) => assignment.status === "stalled" && assignment.recoveryPolicy === "replan");
...
export async function runOverseerTick(opts) {
    ...
    if (!resolved.tickEveryMs)
        return { status: "skipped", reason: "disabled" };
    const queueSize = getQueueSize("main" /* CommandLane.Main */);
    if (queueSize > 0) {
    ...
    try {
        ...
        const assignments = Object.values(storeSnapshot.assignments ?? {}).filter((assignment) => isAssignmentAllowed(resolved, assignment));
        const sampleFor = new Set();
        for (const assignment of assignments) {
