...
function enforcePlanBounds(parsed, maxes) {
    const errors = [];
    ...
    for (const phase of parsed.phases) {
        if (phase.tasks.length > maxes.maxTasks) {
            errors.push(`tasks exceeds maxTasks (${phase.tasks.length} > ${maxes.maxTasks}) in ${phase.id}`);
        }
        for (const task of phase.tasks) {
            if (task.subtasks.length > maxes.maxSubtasks) {
                errors.push(`subtasks exceeds maxSubtasks (${task.subtasks.length} > ${maxes.maxSubtasks}) in ${task.id}`);
            }
        }
    }
    return errors;
...
export async function generateOverseerPlan(params) {
    ...
    for (let attempt = 0; attempt <= maxRepairAttempts; attempt += 1) {
        ...
        if (!parsedResult.success) {
            validationErrors = parsedResult.error.issues.map((e) => e.message);
            continue;
        }
