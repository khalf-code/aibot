...
export function resolveOverseerDir(cfg = loadConfig()) {
...
export function resolveOverseerStorePath(cfg = loadConfig()) {
    return path.join(resolveOverseerDir(cfg), DEFAULT_FILE_NAME);
}
export function createEmptyOverseerStore(now = Date.now()) {
    return {
...
function archiveCorruptStore(pathname) {
    try {
...
function capArray(input, maxItems = MAX_ARRAY_ITEMS) {
...
function capString(input, maxChars) {
...
function sanitizeDispatchHistory(entry) {
    return {
        ...entry,
...
function sanitizeCrystallization(entry) {
    return {
        ...entry,
...
export function loadOverseerStoreFromDisk(cfg = loadConfig()) {
...
export function saveOverseerStoreToDisk(store, cfg = loadConfig()) {
    const pathname = resolveOverseerStorePath(cfg);
    const sanitized = sanitizeStore(store);
    saveJsonFile(pathname, sanitized);
}
export async function updateOverseerStore(fn, cfg = loadConfig()) {
    const pathname = resolveOverseerStorePath(cfg);
