...
import type { ZoidbergBotConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("zoidbergbot-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("zoidbergbot-bin-sherpa-");
      const modelDir = await makeTempDir("zoidbergbot-sherpa-model-");
      ...
      const cfg: ZoidbergBotConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("zoidbergbot-bin-whispercpp-");
      const modelDir = await makeTempDir("zoidbergbot-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("zoidbergbot-bin-gemini-");
      ...
      const cfg: ZoidbergBotConfig = { tools: { media: { image: {} } } };
