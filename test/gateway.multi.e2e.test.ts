...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `zoidbergbot-e2e-${name}-`));
  const configDir = path.join(homeDir, ".zoidbergbot");
  await fs.mkdir(configDir, { recursive: true });
  const configPath = path.join(configDir, "zoidbergbot.json");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ZOIDBERGBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        ZOIDBERGBOT_GATEWAY_TOKEN: inst.gatewayToken,
        ZOIDBERGBOT_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ZOIDBERGBOT_GATEWAY_PORT: String(gwA.port),
          ZOIDBERGBOT_GATEWAY_TOKEN: gwA.gatewayToken,
          ZOIDBERGBOT_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ZOIDBERGBOT_GATEWAY_PORT: String(gwB.port),
          ZOIDBERGBOT_GATEWAY_TOKEN: gwB.gatewayToken,
          ZOIDBERGBOT_GATEWAY_PASSWORD: "",
        }),
