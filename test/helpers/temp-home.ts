...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ZOIDBERGBOT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.ZOIDBERGBOT_STATE_DIR = path.join(base, ".zoidbergbot");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "zoidbergbot-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".zoidbergbot", "agents", "main", "sessions"), {
    recursive: true,
  });
  if (opts.env) {
